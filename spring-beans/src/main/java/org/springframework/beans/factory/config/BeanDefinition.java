 ...
 * BeanDefinition描述一个bean实例，即属性值和构造参数，具体的实现提供的更多信息。
 * <p>This is just a minimal interface: The main intention is to allow a
 * {@link BeanFactoryPostProcessor} to introspect and modify property values
 * and other bean metadata.
 *
 * 从上面可以看出，BeanDefinition接口用于描述一个bean实例的属性及构造参数等元数据；
 * 主要提供了父beanname，bean类型名，作用域，懒加载， bean依赖，自动注入候选bean，自动注入候选主要bean熟悉的设置与获取操作。
 * 同时提供了判断bean是否为单例、原型模式、抽象bean的操作，及获取bean的描述，资源描述，属性源，构造参数，原始bean定义等操作。
 *
...
public interface BeanDefinition extends AttributeAccessor, BeanMetadataElement {
	 ...
	 * 表示一个支持一些比较大的配置的bean定义，
	 * 如一个外部的组件定义，{@link org.springframework.beans.factory.parsing.ComponentDefinition}
	 */
	// TODO 这什么意思？
	int ROLE_SUPPORT = 1;
	 ...
	 * 设置FactoryBean的name，bean的name可以被工厂方法使用。
	 * @see #setFactoryMethodName
	 */
